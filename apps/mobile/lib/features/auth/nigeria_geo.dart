/// Nigeria states + Local Government Areas, used by the sign-up forms so users
/// pick from a list instead of free-typing. State names match the canonical
/// list on the API's `AllowedCountry` row (see packages/db/prisma/seed.ts).
///
/// 36 states + FCT, 774 LGAs. Reference data — safe to treat as constant.
library;

const Map<String, List<String>> nigeriaStatesAndLgas = {
  'Abia': [
...
/// The 36 states + FCT, in the order the API lists them.
const List<String> nigeriaStates = [
...
/// LGAs for [state], or an empty list if unknown.
List<String> lgasFor(String? state) =>
    state == null ? const [] : (nigeriaStatesAndLgas[state] ?? const []);
